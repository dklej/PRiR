...
/* Nieefektywne obliczanie n-tego wyrazu ciągu Fibonacciego */
int fibonacci(int n) {
    if (n == 0 || n == 1) {
        return n;
    }
...
void coordinate_computations(int processes) {
    ...
    // Wylosuj indeksy elementów ciągu Fibonacciego do wyznaczenia
    for (auto & el : tasks) {
        el = distribution(generator); 
    }
    // Dodaj "znacznik końca", czyli empty_task() dla każdego procesu
    for (int p = 1; p < processes; ++p) {
        tasks.push_back(empty_task());
    }
    
    vector<int> proc_to_task_id(processes, -1);  // który proces dostał które zadanie
    ...
    int task_id = 0;
    for (auto task : tasks) {
        // Zacznij od odebrania wiadomości od procesu obliczeniowego z wynikiem
        ...
        // Jeżeli nadawca miał przydzielone zadanie to zapisz jego wynik
    ...
    // Policz sekwencyjnie w celu weryfikacji wyników
